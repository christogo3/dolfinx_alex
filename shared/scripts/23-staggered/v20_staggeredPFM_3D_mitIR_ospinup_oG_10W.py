...
##########################################################################################
# start time loop
def set_fields_for_restart(counter_restart, trestart, u_ls, u_ls_restart, s_ls_restart, u_old_restart, s_old_restart, s_m1_restart, u_m1_restart, u_old, s_ls, s_old, s_zero, s_zero_restart, s_m1, u_m1):
    ...
    while itterschritte <= max_iterschritte and error > 1e-8 and beta > beta_crit and not (cancel_job) and not (cancel_job2): #2
        ...
        if not (restart_solution_u):
            ...
            if case=='nospinup' or case=='afterspinup':
                try:
                    (iters, converged_s) = solver_s.solve(s_ls)
                except RuntimeError:
            ...
            if not (restart_solution_s):
                ...
                # GerasimovDeLorenzis COMPUTE ENERGY NORMS FOR CONVERGENCE
                if itterschritte == 0:
                ...
                if itterschritte - 1 == max_iterschritte:
                    ...
                    counter_restart = set_fields_for_restart(counter_restart, trestart, u_ls, u_ls_restart, s_ls_restart, u_old_restart, s_old_restart, s_m1_restart, u_m1_restart, u_old, s_ls, s_old, s_zero, s_zero_restart, s_m1, u_m1, itterschritte)
                    itterschritte = 0
                    cancel_job2 = True
