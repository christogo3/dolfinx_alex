...
for subfolder in "$TARGET_DIR"/*/; do
  if [ -d "$subfolder" ]; then
    ...
    echo "Running: mpirun -n 10 python3 linearelastic.py"
    mpirun -n 10 python3 linearelastic.py || { echo "linearelastic.py failed"; exit 1; }

    echo "Running: python3 update_trafo.py"
    python3 update_trafo.py || { echo "update_trafo.py failed"; exit 1; }

    echo "Running: make"
    make || { echo "make failed"; exit 1; }

    echo "Running: ./trafo.x"
    ./trafo.x || { echo "./trafo.x failed"; exit 1; }

    echo "Running: python3 print_e_body_2_paraview.py"
    python3 print_e_body_2_paraview.py || { echo "print_e_body_2_paraview.py failed"; exit 1; }

    echo "Running: python3 find_e33.py"
    python3 find_e33.py || { echo "find_e33.py failed"; exit 1; }

    echo "Running: python3 write_e33_to_mesh.py"
    python3 write_e33_to_mesh.py || { echo "write_e33_to_mesh.py failed"; exit 1; }
