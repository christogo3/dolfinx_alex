...
material_properties = mt.AlSi10.get_material_properties()
E_mod = material_properties["Young's Modulus (Pa)"]
nu = material_properties["Poisson's Ratio"]
tens_strength_al_lower, tens_strength_al_upper = material_properties["Tensile Strength (Pa)"]
# Gc_al_lower, Gc_al_upper = mt.AlSi10.calculate_fracture_resistance()

lam_al, mue_al = mt.compute_lame_parameters(E_mod, nu)
...
with dlfx.io.XDMFFile(comm, os.path.join(alex.os.resources_directory,'medium_pores_original_scale.xdmf'), 'r') as mesh_inp: 
...
# elastic constants
lam = dlfx.fem.Constant(domain, lam_al)
mu = dlfx.fem.Constant(domain, mue_al)
...
# compute load depending von critical stress
sig_c = pf.sig_c_quadr_deg(Gc.value,mu.value,epsilon.value) # should be equal to tensile strength
