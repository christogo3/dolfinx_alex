...
#SBATCH -n 96
#SBATCH -e /work/scratch/as12vapa/22-foam-parameter-study-setup/{FOLDER_NAME}/%x.err.%j
#SBATCH -o /work/scratch/as12vapa/22-foam-parameter-study-setup/{FOLDER_NAME}/%x.out.%j
...
working_directory="$HPC_SCRATCH/22-foam-parameter-study-setup/$working_folder_name"

# Navigate to $HPC_SCRATCH
cd $HPC_SCRATCH

# Parameters for simulation_script.py (passed as command-line arguments)
srun -n 96 apptainer exec --bind $HOME/dolfinx_alex/shared:/home --bind "$working_directory:/work" $HOME/dolfinx_alex/alex-dolfinx.sif python3 $working_directory/script.py \
    --mesh_file {MESH_FILE} \
    --lam_param {LAM_PARAM} \
    --mue_param {MUE_PARAM} \
    --Gc_param {GC_PARAM} \
