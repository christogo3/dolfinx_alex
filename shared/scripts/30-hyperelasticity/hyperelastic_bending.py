...
domain = dlfx.mesh.create_rectangle(comm,[np.array([0.0, 0.0]), np.array([L, L/4])], [nx,ny], cell_type=dlfx.mesh.CellType.quadrilateral)
...
tags = pp.tag_part_of_boundary(domain,
                               right,surface_with_traction_tag)
ds_right_tagged = ufl.Measure('ds', domain=domain, subdomain_data=tags)
...
def get_bcs(t):
    ...
    bc_left_x = bc.define_dirichlet_bc_from_value(domain,0.0,0,left,V,-1)
    bc_left_y = bc.define_dirichlet_bc_from_value(domain,0.0,1,left,V,-1)
    ...
    k=0.215
    tau = (t - t0) / k
    t0_star = A * (2.0 * (1.0-tau*tau) * math.exp(-(tau*tau)/2.0)) / (math.sqrt(3*k) * math.sqrt(math.sqrt(math.pi))) 
    
    P0 = dlfx.fem.Constant(domain, np.array([[0.0, t0_star],
                    [t0_star, 0.0]]))
    
    hyperElasticProblem.set_traction_bc(P0=P0,u=u,N=n,ds=ds_right_tagged(surface_with_traction_tag))
