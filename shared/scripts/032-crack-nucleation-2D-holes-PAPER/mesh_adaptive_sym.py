...
# Argument parser
parser = argparse.ArgumentParser(description="Generate a rectangular domain with a half-circle notch (top half only).")
try:
...
notch_center = [width/2.0, 0.0 , 0.0]  # Center of the circle
circle_notch = model.add_disk(notch_center, dhole / 2)

# Subtract the full circle from the top rectangular domain
top_half_domain = model.boolean_difference([top_rectangle], [circle_notch])
...
# Field 1: Fine mesh around the circle (notch)
