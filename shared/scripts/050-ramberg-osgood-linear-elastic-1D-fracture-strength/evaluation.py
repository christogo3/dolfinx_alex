...
path_gc05679 = os.path.join(script_path, 'run_simulation_linear_elastic_graphs.txt')
...
max_pi_gc05679 = data_linear_elastic_gc05679[2].max()
...
print(f"Maximum $\Pi$ (column 2) - linear elastic $J_c=0.5679J_c^{{0}}$: {max_pi_gc05679:.4f}")
...
label_gc05679 = f"linear elastic $J_c=0.5679J_c^{{0}}$,  $\sigma^{{*}}={max_gc05679:.2f}$ / $\sigma_y$"
...
label_pi_gc05679 = f"linear elastic $J_c=0.5679J_c^{{0}}$,  $\Pi^{{*}}={max_pi_gc05679:.2f}/ ( J_c^0L)$"
...
ev.plot_multiple_columns(
    ...
    0, 2,
    output_file_energy,
    legend_labels=[label_pi_gc1, label_pi_gc05679, label_pi_ro],
    usetex=True,
    xlabel="$u_{0} / \sqrt{J_c^{0}{L} / (\mu_0)}$",
    ylabel="$\Pi / ( J_c^0L)$",
    y_range=None,
