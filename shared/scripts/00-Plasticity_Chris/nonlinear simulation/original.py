...
def mat_large_strain_plast_3D(F, mat_par, hist):
    ...
    if f_tr <= 0:
        ...
        mu_stroke = mu_ * (1/3) * np.linalg.trace(b_e_ten)
        n_ = s_ten / (np.linalg.norm(s_ten) + 1*(10**(-8)))
        C_stroke = 2 * mu_stroke * (II_ten - (1 / 3) * np.tensordot(I_ten, I_ten, 0)) - (2 / 3) * np.linalg.norm(s_ten) * (np.tensordot(n_, I_ten, 0) + np.tensordot(I_ten, n_, 0))
    ...
    return S_ten, c_ep_ten, hist_new
