...
# Output setup
postprocessing_interval = dlfx.fem.Constant(domain,2.0) # Every X steps the fields get printed to XDMF
...
def get_residuum_and_gateaux(delta_t: dlfx.fem.Constant):
    [Res, dResdw] = plasticityProblem.prep_newton(u=u,du=du,ddu=ddu,lam=la,mu=mu) 
    return [Res, dResdw]
...
def after_timestep_success(t,dt,iters):
    ...
    problem_S = alex.plasticity.linear_problem(TEN,dx,S,deg_quad)
    S_interpolated = problem_S.solve()
    ...
    # 3. ------------------------------ Determine Plasticity measure ------------------------------
    #problem_alpha = alex.plasticity.linear_problem(TEN,dx,alpha_n,deg_quad) # alpha as plasticity measure
    #alpha_interpolated = problem_alpha.solve()
    #alpha_interpolated.name = "alpha"
