...
h_cell = w_cell
l0 = (Nholes + 2) * w_cell
h0 = 20.0  # Adjust height to accommodate 3 rows
n_rows = 3
...
# Generate unit cells without holes
cell_matrix_list = []
for i in range(n_rows):  # Iterate over rows
    for n in range(Nholes):  # Iterate over columns
        ...
        cell_matrix_list.append(cell_matrix_tmp)

# Define effective matrix and cell matrix separately
eff_matr_group = model.boolean_fragments([eff_matr], cell_matrix_list, delete_first=False, delete_other=False)
...
refinement_points += crack_points
...
model.add_physical(cell_matrix_list, "cell_matrix")
