...
N = 16
mesh = dolfinx.mesh.create_unit_cube(comm,N,N,N,cell_type=dolfinx.mesh.CellType.hexahedron)
...
δδP = ufl.TrialFunction(Tf)
δδh = ufl.TrialFunction(Hf)
δδB = ufl.TrialFunction(Tf.clone())  # to be distinct from δP

# Define state and variation of state as (ordered) list of functions
m, δm, δδm = [u, P, h, B], [δu, δP, δh, δB], [δδu, δδP, δδh, δδB]
...
Res = ufl.derivative(F,m,δm)
dRes = ufl.derivative(Res,m,δδm)
...
ofile = dolfinx.io.XDMFFile(comm, os.path.join(script_path,f"{name}.xdmf"), "w")
# Write mesh, meshtags
# ofile.write_mesh_meshtags(mesh, mts)

# alex.postprocessing.write_mesh_and_get_outputfile_xdmf(mesh,outputfile_xdmf_path,comm)
...
for step, factor in enumerate(cycles):
    ...
    left_dofs_x = dolfinx.fem.locate_dofs_topological(Uf.sub(0),fdim,left_facets)
    left_dofs_y = dolfinx.fem.locate_dofs_topological(Uf.sub(1),fdim,left_facets)
    left_dofs_z = dolfinx.fem.locate_dofs_topological(Uf.sub(2),fdim,left_facets)
    
    
    right_dofs_x = dolfinx.fem.locate_dofs_topological(Uf.sub(0),fdim,right_facets)
    right_dofs_y = dolfinx.fem.locate_dofs_topological(Uf.sub(1),fdim,right_facets)
    right_dofs_z = dolfinx.fem.locate_dofs_topological(Uf.sub(2),fdim,right_facets)
    
    bc_left_x = dolfinx.fem.dirichletbc(-0.1,left_dofs_x, Uf.sub(0))
    bc_left_y = dolfinx.fem.dirichletbc(0.0,left_dofs_y, Uf.sub(1))
    bc_left_z = dolfinx.fem.dirichletbc(0.0,left_dofs_z, Uf.sub(2))
     
    bc_right_x = dolfinx.fem.dirichletbc(0.1,right_dofs_x, Uf.sub(0))
    bc_right_y = dolfinx.fem.dirichletbc(0.0,right_dofs_y, Uf.sub(1))
    bc_right_z = dolfinx.fem.dirichletbc(0.0,right_dofs_z, Uf.sub(2))
    ...
    problem = NonlinearProblem(Res, m, bcs, dRes)
