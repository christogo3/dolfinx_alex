...
s_tr_np1 = fem.Function(W)

e_p_n = fem.Function(W)
e_p_np1 = fem.Function(W)
...
def constitutive_update(Δε, e_p_n, alpha_n):
    # new_sig, n_elas, beta, dgamma = alex.plasticity.constitutive_update(Δε, as_3D_tensor(old_sig), alpha_n,sig0,H,lmbda,mu)
    sigma_np1_, N_tr_, e_p_np1_, alpha_np1_, dGamma_, s_tr_np1_ = alex.plasticity.constitutive_update_alt(Δε, as_3D_tensor(e_p_n), alpha_n,sig0,H,lmbda,mu)
    return to_vect(sigma_np1_), to_vect(N_tr_), to_vect(e_p_np1_), alpha_np1_, dGamma_, to_vect(s_tr_np1_)
    # sig_n = as_3D_tensor(old_sig)
    # sig_tr = sig_n + sig_el(Δε)
    # sig_dev_tr = ufl.dev(sig_tr)
    # sig_eq = ufl.sqrt(3.0 / 2.0 * ufl.inner(sig_dev_tr, sig_dev_tr))
    # f_tr = sig_eq - ( sig0 + H * alpha_n )
    # dgamma = pos(f_tr) / (3 * mu + H)
    # n_elas = sig_dev_tr / sig_eq * pos(f_tr) / f_tr
    # beta = 3 * mu * dgamma / sig_eq
    # new_sig = sig_tr - beta * sig_dev_tr
    
    # f_tr = ufl.sqrt(ufl.inner(sig_dev_tr,sig_dev_tr))- ufl.sqrt(2. / 3.) * ( sig0 + H * alpha_n )
    ...
    # return to_vect(new_sig), to_vect(n_elas), beta, dgamma
...
def sigma_tang(eps):
    return alex.plasticity.sigma_tang_alt(eps,as_3D_tensor(N_np1),mu,lmbda,H,dGamma,s_tr_np1)
    
    # return alex.plasticity.sigma_tang(eps,lmbda,mu,as_3D_tensor(N_np1),beta,H)
...
# def sigma_tang_alt(eps):
#     N_elas = as_3D_tensor(n_elas)
#     return (
#         sig_el(eps)
#         - (2. * mu) ** 2 / (2. * mu +  H * 2. / 3.) * ufl.inner(N_elas, eps) * N_elas
...
e_p_n.vector.set(0.0)
s_tr_np1.vector.set(0.1)
dGamma.vector.set(0.0)


Δε = eps(Du)
# sig_, n_elas_, beta_, dgamma_ = constitutive_update(Δε, sig_old, alpha_n)

sig_, N_np1_, e_p_np1_, alpha_np1_, dGamma_, s_tr_np1_ = constitutive_update(Δε,e_p_n,alpha_n)
...
for i, t in enumerate(load_steps):
    ...
    while nRes / nRes0 > tol and niter < Nitermax:
        ...
        interpolate_quadrature(N_np1_, N_np1)
        interpolate_quadrature(dGamma_, dGamma)
        interpolate_quadrature(s_tr_np1_, s_tr_np1)
    ...
    interpolate_quadrature(dGamma_, dGamma)
    alpha_n.vector.axpy(1, math.sqrt(2.0 / 3.0 ) * dGamma.vector)
    
    interpolate_quadrature(e_p_np1_, e_p_np1)
    e_p_n.x.array[:] = e_p_np1.x.array[:]
