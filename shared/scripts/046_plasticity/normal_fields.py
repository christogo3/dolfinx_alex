...
sig_y = dlfx.fem.Constant(domain, 0.5)
...
Te = basix.ufl.element("P", domain.basix_cell(), 1, shape=(2,2))# tensor fields
W = dlfx.fem.functionspace(domain, basix.ufl.mixed_element([Ve, Se]))

T = dlfx.fem.functionspace(domain,Te)
...
alpha_n = dlfx.fem.Function(SS)
alpha_tmp = dlfx.fem.Function(SS)
e_p_n = dlfx.fem.Function(T)

alpha_n.x.array[:] = np.zeros_like(alpha_n.x.array[:])
alpha_tmp.x.array[:] = np.zeros_like(alpha_tmp.x.array[:])
e_p_n.x.array[:] = np.zeros_like(e_p_n.x.array[:])
...
phaseFieldProblem = pf.StaticPhaseFieldProblem2D_incremental_plasticity(degradationFunction=pf.degrad_cubic,
                                                   psisurf=pf.psisurf_from_function,dx=dx, sig_y=sig_y.value, hard=hard.value,alpha_n=alpha_n,e_p_n=e_p_n,Id=Id)
...
def after_timestep_success(t,dt,iters):
    ...
    e_p_n.interpolate(e_p_expr,T.element.interpolation_points())
    
    #e_p_n.x.array[:] = alex.plasticity.interpolate_quadrature(domain, cells, quadrature_points,e_p_expr)
