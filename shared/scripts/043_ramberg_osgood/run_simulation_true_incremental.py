...
delta_w =  dlfx.fem.Function(W)
delta_u,delta_s = delta_w.split()
...
w_S = dlfx.fem.Function(W)
_,sm1 = ufl.split(wm1)
bc_expression_s = dlfx.fem.Expression(ufl.as_ufl(0.0)-sm1,W.sub(1).element.interpolation_points())
...
Id = ufl.as_matrix(((1,z),
                    (z,1)))
phaseFieldProblem = pf.StaticPhaseFieldProblem2D_true_incremental(degradationFunction=pf.degrad_quadratic,
...
def get_residuum_and_gateaux(delta_t: dlfx.fem.Constant):
    [Res, dResdw] = phaseFieldProblem.prep_newton(
        delta_w=delta_w,wm1=wm1,dw=dw,ddw=ddw,lam=la, mu = mu,
...
def compute_surf_displacement():
    ...
    return ufl.as_vector([u_x, u_y])-um1 # only 2 components in 2D
...
def get_bcs(t):
    ...
    w_S.sub(1).interpolate(bc_expression_s)
    bccrack : dlfx.fem.DirichletBC = dlfx.fem.dirichletbc(w_S,crackdofs)
...
def after_timestep_success(t,dt,iters):
    ...
    w.x.array[:] = delta_w.x.array[:] + wm1.x.array[:]
    ...
    # update
    wm1.x.array[:] = wm1.x.array[:] + delta_w.x.array[:]
    #delta_wrestart.x.array[:] = delta_w.x.array[:]
...
def after_timestep_restart(t,dt,iters):
    #w.x.array[:] = wrestart.x.array[:]
    delta_w.x.array[:] = np.zeros_like(delta_w.x.array[:]) #wrestart.x.array[:]
