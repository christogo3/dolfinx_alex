...
for folder_path in "${BASE_DIR}"/simulation_*; do
    if [ -d "${folder_path}" ]; then
        ...
        IFS=' ' read -r mesh_file lam_param mue_param gc_param eps_factor_param element_order <<< "$params"

        # Get memory, processor, and time settings based on mesh_file
        mem_proc_time=$(get_memory_processors_and_time "$mesh_file")
        IFS=' ' read -r memory_value processor_number time <<< "$mem_proc_time"

        # Call generate_job_script with extracted parameters and memory/processor/time settings
        generate_job_script "${folder_name}" "${mesh_file}" "${mesh_file}" "${lam_param}" "${mue_param}" "${gc_param}" "${eps_factor_param}" "${element_order}" "${memory_value}" "${processor_number}" "${time}"
    fi
done
