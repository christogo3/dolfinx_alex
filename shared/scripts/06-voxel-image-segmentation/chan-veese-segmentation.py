...
voxel_file_in_path  = os.path.join(script_path,"output_smooth.dat")
...
# creating fem mesh
domain : dlfx.mesh.Mesh = dlfx.mesh.create_unit_cube(comm,voxel_number_x_reduced,voxel_number_x_reduced,voxel_number_x_reduced,cell_type=dlfx.mesh.CellType.hexahedron)
...
c1 = dlfx.fem.Constant(domain, 0.0)
c2 = dlfx.fem.Constant(domain, 1.0)
...
def before_first_timestep():
    alex.postprocessing.write_meshoutputfile(domain, outputfile_xdmf_path, comm,meshtags=cell_tags)
...
def after_last_timestep():
    ...
    img.write_field_to_voxel_data_leS(domain, fem2voxel_file_path, voxel_number_x_reduced, segmentation_as_cell_field)
